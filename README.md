...
- Streamlit-UI для загрузки файлов, обновления индекса, просмотра текущего состояния и запуска генерации ответа.
...
```
UI включает переключатель «Поиск» / «Поиск + генерация». В генеративном режиме укажите путь к GGUF-модели
(phi-2, mistral, zephyr, tinyllama) для llama-cpp-python. На вход передаются топ-N найденных чанков и вопрос;
LLM возвращает короткий ответ с обязательной ссылкой на `ticket_id`. Ограничивайте `max_tokens` (512–1024), чтобы
уложиться в ~10 секунд на CPU. Все модели используют одинаковый промпт.

### 5) Дозапись новых тикетов в существующий индекс
```bash
...
### 6) Streamlit-интерфейс для индексации
UI:
...
- **Генерация ответа**: GGUF-модели (phi-2, mistral, zephyr, tinyllama) через `llama-cpp-python` с единым промптом.
...
rag_agent/
  ...
  generator.py     # промпт и вызов llama-cpp для генерации ответа из найденных чанков
